...
class RareItemsFilter {
  ...
  static isTanningMirror(itemName) {
    return itemName?.toLowerCase() === 'tanning mirror';
  }
  ...
  static filterRareItems(stockData) {
    ...
    // Filter GEAR: sprinkler ATAU Tanning Mirror
    const gearItems = stockData.Data?.gear || [];
    gearItems.forEach(item => {
      if ((this.isSprinkler(item.name) || this.isTanningMirror(item.name)) && parseInt(item.stock) > 0) {
        rareStock.gear.push({
          name: item.name,
          stock: item.stock,
          rarity: item.rarity
