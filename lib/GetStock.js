...
// Buat lookup map untuk rarity berdasarkan item_id dan display_name
function createItemLookup() {
  ...
  items.forEach(item => {
    ...
    // Lookup berdasarkan item_id
    if (item.item_id) {
      itemLookup[item.item_id] = { rarity };
    }
    
    // Lookup berdasarkan display_name
    if (item.display_name) {
      itemLookup[item.display_name] = { rarity };
    }
    
    // Lookup berdasarkan normalized item_id
    if (item.item_id) {
...
function formatStockData(wsData, itemLookup) {
  const formatCategory = (categoryData) => {
    ...
    return categoryData.map(item => {
      // Coba lookup berdasarkan berbagai identifier
      ...
      const finalRarity = rarityFromItemId || rarityFromDisplayName || rarityFromNormalizedItemId || rarityFromNormalizedDisplayName || 'unknown';
